'''!@namespace modules.transform

@brief Function to transform user defined input path for compatibility with solver
...
def transform_path(path):
    """!@brief Processes user defined input path by interpolating (in 
    a manner that creates a higher density of points at early times) and 
    smoothing using a savitzky-golay filter for compatibility with solver

    @param path: Matrix representation of the path. A nd.ndarray of dimension (n,2)
  
    @return Transformed path. An nd.array of shape (x,2) where x is 
    greater than the original length of the input path, as the length of the array 
    is expanded during the 'transformation'

    @return dt. Step size of time.
